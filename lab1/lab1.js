...
//Using parseInt function to convert the price string to an integer.
//Also using replace function to remove the "$" and "," characters from the string price.

let price = parseInt(listings.price.replace(/[$,]/g, ""));
let fees = price <= 450000 ? price * 0.025 : price * 0.02;
return fees;
...
/**
 * Task-5: Sort the listing array ascendingly (from Cheap to Expensive) in a new variable called listingAscendingly
 */
//WRITE YOUR CODE BELOW
...
const newBuiltListings = listings.filter(listing => parseInt(listing.propertySummary.builtIn) >= 1990);
